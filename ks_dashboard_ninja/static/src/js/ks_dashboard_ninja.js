...
//            ".ks_date_input_fields"
        ...
        _renderChart($ks_gridstack_container,item){
            ...
            for(var i=0;i<chart_data['labels'].length;i++){
                if(chart_data['labels'][i] === false){
                    chart_data['labels'][i] = undefined
                }
            }
...
//                                                        ks_type = field_utils.format.char(ks_field);
        ...
        convertStringToFloat: function(stringValue) {
                    ...
                    for (var i = 0; i < list_view_data.data_rows.length; i++){
                        for (var j = 0; j < list_view_data.data_rows[0]["data"].length; j++){
                            if(typeof(list_view_data.data_rows[i].data[j]) === "number" || list_view_data.data_rows[i].data[j]){
                                ...
                                list_view_data.data_rows[i].data[j] = 'undefined';
                            }
                            if (j in total){
                                total[j]  = total[j] + self.convertStringToFloat(list_view_data.data_rows[i].data[j]);
                            }else{
                                total[j] = self.convertStringToFloat(list_view_data.data_rows[i].data[j]);
                            }
                        }
                    }
                }
                total[0] = 'Total';
                var values = $.map(total, function(key, value) { return self.addCommas(key) });
                if(values.length > 1){
                    list_view_data['data_rows'].push({'data': values});
                }
                console.log("list_view_data['data_rows'] after", list_view_data['data_rows']);
